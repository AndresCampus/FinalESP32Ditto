...
int publish_delta_DP = 100; // Parametro de salto configurable
int threshold_vent_DP = 1000; // Umbral para arrancar ventilación
int auto_mode_DP = 0; // 0 = Manual, 1 = Automático
int vent_relay_DP = 0; // Estado real de los ventiladores (0 = OFF, 1 = ON)
...
void procesa_mensaje(char* topic, byte* payload, unsigned int length) { 
  ...
  // Verificamos si es un mensaje de cambio de Estado / Parámetros (Desired Properties)
    ...
    if (doc.containsKey("vent_relay_DP")) {
      ...
      if (auto_mode_DP == 0) { // Solo si estamos en MODO MANUAL actuamos físicamente
        vent_relay_DP = nuevo_vr;
        digitalWrite(RELAYPIN, vent_relay_DP ? HIGH : LOW);
        digitalWrite(LEDPIN, vent_relay_DP ? HIGH : LOW);
        Serial.println(DEBUG_STRING + ">>> [DESIRED] Relé forzado a " + String(vent_relay_DP));
    ...
    if (doc.containsKey("auto_mode_DP")) {
      ...
      Serial.println(DEBUG_STRING + ">>> [DESIRED] Auto_mode asignado a " + String(auto_mode_DP));
    ...
    if (doc.containsKey("threshold_vent_DP")) {
      ...
      Serial.println(DEBUG_STRING + ">>> [DESIRED] Umbral de Alarma fijado en " + String(threshold_vent_DP) + " ppm");
    ...
    if (doc.containsKey("publish_delta_DP")) {
      ...
      Serial.println(DEBUG_STRING + ">>> [DESIRED] Delta rápido fijado en " + String(publish_delta_DP) + " ppm");
  ...
  while(true) {
    ...
    if (auto_mode_DP == 1) {
      ...
      if (deseado != vent_relay_DP) {
        ...
        digitalWrite(RELAYPIN, vent_relay_DP ? HIGH : LOW);
        digitalWrite(LEDPIN, vent_relay_DP ? HIGH : LOW); // LED chivato secundario
        
        Serial.println(DEBUG_STRING + ">>> AUTÓMATA: Cambio de estado de ventilación a: " + String(vent_relay_DP));
...
void clickCorto(Button2& btn) {
  ...
  if (auto_mode_DP == 0) {
    vent_relay_DP = (vent_relay_DP == 1) ? 0 : 1; // Alternamos la placa física
    digitalWrite(RELAYPIN, vent_relay_DP ? HIGH : LOW);
    digitalWrite(LEDPIN, vent_relay_DP ? HIGH : LOW);
    
    Serial.println(DEBUG_STRING + ">>> RELÉ CONMUTADO MANUALMENTE A: " + String(vent_relay_DP));
...
void clickLargo(Button2& btn) {
  auto_mode_DP = (auto_mode_DP == 1) ? 0 : 1; // Alternamos el modo de control global
